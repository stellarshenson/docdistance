...
- **Length-bucketing (E05-H23)** - the reranker pairs carry 2.82x padded-token waste (median 57 tokens padded to 256); length-sorting the grid and tightening `max_length` to the per-call 100th-percentile cuts the CPU INT8 reranker ~43% (73.2 s → 42.0 s on one document) at bit-identical scores - portable, zero correctness risk, the one E05 speed win
